...
public class RegistrationService {
	...
	public Response createMember(UserRegistration request) {
		if (!request.getPassword().equals(request.getPasswordConfirmation())) {
			return MessageBuilder.badRequest().message("Password mismatch.")
					.build();
		}
		...
		try {
			...
			if (this.identityModelManager.findByLoginName(request.getEmail()) == null) {
				HdUser newUser = this.identityModelManager
				...
				message = MessageBuilder.badRequest().message(
	...
	public Response activateAccount(String activationCode) {
		...
		try {
			Token token = this.identityModelManager
	...
	private void sendNotification(UserRegistration request,
			String activationCode) {
		// Email email = new Email("Please complete the signup",
		// "http://localhost:8080/picketlink-angularjs-rest/#/activate/"
		// + activationCode, request.getEmail());
		//
		// event.fire(email);
	}
}
