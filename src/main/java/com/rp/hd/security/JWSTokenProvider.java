...
/**
 * <p>
 * This is a simple {@link org.picketlink.idm.credential.Token.Provider} that
 * manages a specific token type. In this case the type is
 * {@link com.rp.hd.security.JWSToken}
 * .
 * </p>
 *
 *
 * @see com.rp.hd.security.JWSToken
 */
@Stateless
public class JWSTokenProvider implements Token.Provider<JWSToken> {
	...
	public JWSToken issue(Account account) {
		...
		// here we construct a JWS signed with the private key provided by the
		// partition.
		builder.id(UUID.randomUUID().toString()).rsa256(privateKey)
		...
		// now we update the account with the token previously issued by this
		// provider.
