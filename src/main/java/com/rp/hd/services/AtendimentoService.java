...
public class AtendimentoService {
	...
	public List<ModeloFoto> getFotosModelos() {
		return modeloConviteRepository.getModelosComFotos().stream().map(modelo -> {
			if (modelo.getFotos().size() > 0) {
				String caminho =  modelo.getFotos().stream().sorted((foto1,foto2) -> {
					return foto1.getOrdem() - foto2.getOrdem();
				}).collect(Collectors.toList()).get(0).getCaminho();
				
				ModeloFoto m = new ModeloFoto(modelo.getId(),modelo.getNome(), caminho);
				return m;
			} else {
				ModeloFoto m = new ModeloFoto(modelo.getId(),modelo.getNome(), null);
				return m;
			}
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public List<ModeloConvite> getFotosModelo(@PathParam("modelo") Long modelo) {
		return modeloConviteRepository.getModeloComFotos(modelo);		
	}
...
//	@Consumes(MediaType.APPLICATION_JSON)
//	@Produces(MediaType.APPLICATION_JSON)
//	public void adicionarOrcmaneto(@PathParam("atendimento") Long atendimentoId, SolicitacaoOrcamento sol) {
