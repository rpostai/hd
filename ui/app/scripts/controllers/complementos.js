...
function ComplementosController($scope, $http, store) {
  ...
  $scope.verComplementos = function() {
	  ...
	  $http.get("/servicos/atendimento/complementos/"+atendimentoSalvo.id).success(function(data) {
		  $scope.dados = data;
		  $scope.valorTotal = 0;
		  ...
		  $http.get("/servicos/atendimento/complementosatendimento/"+atendimentoSalvo.id).success(function(data) {
  ...
  $scope.adicionar = function(item,valor) {
	  ...
	  if (quantidade > 0) {
		  $http.post("/servicos/atendimento/complementossalvar/"+atendimentoSalvo.id+"/"+item+"/"+quantidade).success(function(data) {
